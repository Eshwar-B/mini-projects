...
uploaded_file = st.file_uploader("stocks.csv")
if uploaded_file is not None:
    x = pd.read_csv(uploaded_file)
    
    # Ensure 'Date' column is in proper format 'dd-mm-yyyy'
    x['Date'] = pd.to_datetime(x['Date'], format='%d-%m-%Y', errors='coerce')
    
    for column in x.columns:
        x.rename(columns={column: column.title()}, inplace=True)
    
    # Filter out any rows where 'Date' is NaT (could happen if there are parsing errors)
    ...
    # Deep learning model
    model = load_model(r'D:\mini project\stock analysis\ta.h5')
    ...
    if abc is not None:
        ...
        fig3 = go.Figure(data=[go.Line(x=results.index, y=x[abc]), 
                               go.Line(x=results.index, y=results['Forecast'])])
